...
type SecureDropMetadata struct {
	...
	Platform           string             `json:"server_os" csv:"server_os"`
	...
	V2SourceURL        string             `json:"v2_source_url" csv:"v2_source_url"`
	V3SourceURL        string             `json:"v3_source_url" csv:"v3_source_url"`
}
...
func checkStatus(wg *sync.WaitGroup, results chan ScanResult, updates chan string, client *http.Client, sd SecureDrop) {
	...
	result.DirectoryURL = sd.DirectoryURL
	result.LandingPageURL = sd.LandingPageURL
...
func scan(cmd *cli.Cmd) {
	...
	var (
		...
		format     = cmd.StringOpt("f format", "json", "Specify output format: \"csv\", \"json\", or \"pp\"")
		...
		verbose    = cmd.BoolOpt("v verbose", false, "Provide more detail about scan progress")
	...
	cmd.Action = func() {
		...
		if *format != "csv" && *format != "json" && *format != "pp" {
